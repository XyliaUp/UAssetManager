...
using static UAssetAPI.Pak.Objects.Core.Misc.ECompressionFlags;
using static UAssetAPI.Pak.Objects.EPakFileVersion;
using static UAssetAPI.Pak.Versions.EGame;
...
public class FPakEntry : VfsEntry
{
    ...
    public FPakEntry(PakFileReader reader, string path, FArchive Ar) : base(reader, path)
    {
        ...
        if (reader.Info.Version < PakFile_Version_FNameBasedCompressionMethod)
        {
            ...
            else
            {
                ...
                {
                    compressionMethodIndex = 3; // PUBG uses Zlib compression method
                }
    ...
    public unsafe FPakEntry(PakFileReader reader, string path, byte* data) : base(reader, path)
    {
        ...
        else if (compressionBlocksCount > 0)
        {
            ...
            var compressedBlockAlignment = IsEncrypted ? Encryption.Aes.Aes.ALIGN : 1;
    ...
    public FPakEntry(PakFileReader reader, FMemoryImageArchive Ar) : base(reader)
    {
        ...
        // Take into account CompressionBlocks
        if (CompressionMethod != CompressionMethod.None)
            StructSize += sizeof(int) + CompressionBlocks.Length * 2 * sizeof(long);
    }
    ...
    {
        // If this is a build entry with data already in memory, return it directly
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public FPakEntry(PakFileReader reader, byte[] buffer, string vfsPath, CompressionMethod method = CompressionMethod.None) : base(reader, vfsPath)
    {
        RawData = buffer;
        Path = vfsPath;
        ...
        if (method == CompressionMethod.None)
        {
            ...
            Data = reader.EncryptIfEncrypted(buffer, IsEncrypted);
            SHAHash = SHA1.HashData(Data.SubByteArray((int)UncompressedSize));
            CompressionBlockSize = 0;
        }
        else
        {
            ...
            SHAHash = SHA1.HashData(buffer);

            var blocks = new List<KeyValuePair<FPakCompressedBlock, byte[]>>();
            CompressionBlockSize = Math.Min((uint)UncompressedSize, 1048576);

            // split data blocks
            using var tempReader = new BinaryReader(new MemoryStream(buffer));
            while (tempReader.BaseStream.Position < tempReader.BaseStream.Length)
            {
                var blockLength = (int)Math.Min(CompressionBlockSize, tempReader.BaseStream.Length - tempReader.BaseStream.Position);
                var blockData = reader.EncryptIfEncrypted(UAssetAPI.Pak.Compression.Compression.Compress(tempReader.ReadBytes(blockLength), blockLength, method), IsEncrypted);

                blocks.Add(new(new FPakCompressedBlock()
                {
                    CompressedStart = CompressedSize,
                    CompressedEnd = CompressedSize + blockData.Length,
                }, blockData));

                CompressedSize += blockData.Length;
            }

            CompressionBlocks = blocks.Select(o => o.Key).ToArray();
            CompressionBlocksData = blocks.Select(o => o.Value).ToArray();
        }
    }
    ...
    public override void WriteInfo(FPakInfo info, BinaryWriter writer, bool WriteOffset)
    {
        ...
        writer.Write(WriteOffset ? Offset : 0L);
        writer.Write(CompressedSize);
        writer.Write(UncompressedSize);

        #region CompressionMethod
        var CompressionMethod = IsCompressed ? this.CompressionMethod : Compression.CompressionMethod.None;
        ...
        else if (info.Version == PakFile_Version_FNameBasedCompressionMethod && !info.IsSubVersion)
        {
            if (!info.CompressionMethods.Contains(CompressionMethod))
        ...
        if (info.Version < PakFile_Version_NoTimestamps)
            writer.Write(Timestamp);
        writer.Write(SHAHash);
        #endregion

        #region CompressionBlock
        if (info.Version >= PakFile_Version_CompressionEncryption)
        {
            if (CompressionMethod != CompressionMethod.None)
            {
                writer.Write(CompressionBlocks.Length);
                foreach (var CompressionBlock in CompressionBlocks)
                {
                    if (info.Version >= PakFile_Version_RelativeChunkOffsets)
                    {
                        writer.Write(CompressionBlock.CompressedStart);
                        writer.Write(CompressionBlock.CompressedEnd);
                    }
                    else
                    {
    ...
    public override void Write(FPakInfo info, BinaryWriter writer)
    {
        ...
        #region StructSize
        using var tempWriter = new BinaryWriter(new MemoryStream());
        WriteInfo(info, tempWriter, false);

        var StructSize = (int)tempWriter.BaseStream.Length;
        for (int i = 0; i < CompressionBlocks.Length; i++)
        {
            CompressionBlocks[i].CompressedStart += StructSize;
            CompressionBlocks[i].CompressedEnd += StructSize;
        }
        #endregion

        WriteInfo(info, writer, false);
        if (!IsCompressed) writer.Write(Data);
        else Array.ForEach(CompressionBlocksData, writer.Write);
    }
