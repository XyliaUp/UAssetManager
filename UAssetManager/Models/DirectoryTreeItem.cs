...
public partial class DirectoryTreeItem : ObservableObject
{
	...
	public DirectoryTreeItem(string name, string fullPath, bool isFile, DirectoryTreeItem? parent)
	{
		_name = name;
		_fullPath = fullPath;
		_isFile = isFile;
...
public class DirectoryTree
{
	...
	public DirectoryTreeItem? CreateNode(string path, string? fixedAssetOnDisk = null, string? prefix = null, GameFile? gameFile = null, List<GameFile>? gameFiles = null)
	{
		...
		if (!RootNodes.TryGetValue(pathComponents[0], out DirectoryTreeItem? currentItem))
		{
			...
			currentItem = new DirectoryTreeItem(pathComponents[0], pathComponents[0], ext.Length > 1, null);
		...
		for (int i = 1; i < pathComponents.Length; i++)
		{
			...
			if (!currentItem.Children.TryGetValue(pathComponents[i], out DirectoryTreeItem? value))
			{
				currentItem.Children[pathComponents[i]] = value = new DirectoryTreeItem(
					pathComponents[i],
