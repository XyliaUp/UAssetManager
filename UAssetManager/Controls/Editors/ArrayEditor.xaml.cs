...
public partial class ArrayEditor
{
    ...
    private void InsertArrayElementAt(int insertIndex)
    {
        if (Array == null || Asset == null) return;
        var length = Array.Value?.Length ?? 0;
        insertIndex = Math.Clamp(insertIndex, 0, length);
        var exemplar = Array.Value!.LastOrDefault();
        var newElem = exemplar != null
            ? CreateDefaultElementLike(exemplar, Asset, false)
            : new StrPropertyData(FName.DefineDummy(Asset, "0")) { Value = new FString(string.Empty) };
        Array.Value = Array.Value!.Take(insertIndex).Concat([newElem]).Concat(Array.Value!.Skip(insertIndex)).ToArray();
    }
    ...
    private static PropertyData CreateDefaultElementLike(PropertyData exemplar, UAsset asset, bool copy)
    {
        return exemplar switch
        {
            ...
            StrPropertyData p => new StrPropertyData(exemplar.Name) { Value = new FString(copy ? p.Value?.Value ?? string.Empty : string.Empty) },
            NamePropertyData p => new NamePropertyData(exemplar.Name) { Value = copy ? p.Value : FName.DefineDummy(asset, "None") },
            BytePropertyData p when p.ByteType == BytePropertyType.FName => new BytePropertyData(exemplar.Name) { ByteType = BytePropertyType.FName, EnumValue = copy ? p.EnumValue : FName.DefineDummy(asset, "None") },
            ...
            StructPropertyData p => new StructPropertyData(exemplar.Name, p.StructType) { Value = copy ? [.. p.Value.Select(child => CreateDefaultElementLike(child, asset, true))] : [] },
            _ => new StrPropertyData(exemplar.Name) { Value = new FString(string.Empty) }
        };
    }
    #endregion
}
