...
using static UAssetManager.Pak.Objects.Core.Misc.ECompressionFlags;
using static UAssetManager.Pak.Pak.Objects.EPakFileVersion;
using static UAssetManager.Pak.Versions.EGame;

namespace UAssetManager.Pak.Pak.Objects;

internal class FPakEntry : VfsEntry
{
	...
	public FPakEntry(PakFileReader reader, byte[] buffer, string vfsPath, CompressionMethod method = CompressionMethod.None) : base(reader, vfsPath)
	{
		...
		else
		{
			...
			while (tempReader.BaseStream.Position < tempReader.BaseStream.Length)
			{
				...
				var blockData = reader.EncryptIfEncrypted(Compression.Compression.Compress(tempReader.ReadBytes(blockLength), blockLength, method), IsEncrypted);
