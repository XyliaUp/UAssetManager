...
internal class FloatPropertyEditor : PropertyEditorBase, IValueConverter
{
    ...
    {
        if (parameter is FloatPropertyData)
        {
            return (value is string str && float.TryParse(str.Trim(), NumberStyles.Float, culture, out float result)) ? result : 0.0f;
        }
        if (parameter is DoublePropertyData)
        {
			return (value is string str && double.TryParse(str.Trim(), NumberStyles.Float, culture, out double result)) ? result : 0.0d;
        }
