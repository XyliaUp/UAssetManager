...
public partial class ObjectReferenceDialog : Window
{
	...
	private void AddReferenceButton_Click(object sender, RoutedEventArgs e)
	{
		...
		if (inputDialog.ShowDialog() == true)
		{
			...
			try
			{
				var result = ParseAndAddReference(_asset, fullPath);
				...
				var newItem = _items.FirstOrDefault(item => item.PackageIndex.Index == result);
				if (newItem != null)
				{
			...
			{
				MessageBox.Show(StringHelper.Get("ObjectReferenceDialog.PathParseError", ex.Message), StringHelper.Get("Text.Error"), MessageBoxButton.OK, MessageBoxImage.Error);
			}
		}
	}
	...
	public static int ParseAndAddReference(UAsset asset, string fullPath)
	{
		// Validate path format
		string pattern = @".*?\/content\/(?=.)?";
		var parts = Regex.Replace(fullPath, pattern, "/Game/", RegexOptions.IgnoreCase).Split('.');
		if (parts.Length < 2)
		...
		{
			var part = parts[i];
			if (part.Equals("uasset", StringComparison.OrdinalIgnoreCase) && i == parts.Length - 1)
			{
				part = parts[i - 1][(parts[i - 1].LastIndexOf('/') + 1)..];
			}

			currentIndex = FindOrCreateObject(asset, part, currentIndex, isExport, i == 0);
		}

		return currentIndex;
	}

	private static int FindOrCreateObject(UAsset asset, string objectName, int outerIndex, bool isExport, bool isPackage)
	{
		if (isExport)
		{
			...
			{
				ObjectName = FName.FromString(asset, objectName),
