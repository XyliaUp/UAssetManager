...
using UAssetAPI.Pak.FileProvider;
...
namespace CUE4Parse.FileProvider.Vfs
{
	public abstract class AbstractVfsFileProvider : AbstractFileProvider, IVfsFileProvider
	{
		...
		public GameFile this[string path, string archiveName, StringComparison comparison = StringComparison.OrdinalIgnoreCase] => this[path, GetArchive(archiveName, comparison)];
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public UAsset LoadPackage(string path, IAesVfsReader archive) => LoadPackage(this[path, archive]);
